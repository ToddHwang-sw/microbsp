...
#ifdef XCONFIG_PATCH
	if (NULL == (key = (struct MHD_IPCount *)os_malloc (sizeof(*key))))
#else
...
#ifdef XCONFIG_PATCH
	if (NULL == (connection = (struct MHD_Connection *)os_calloc(1, sizeof (struct MHD_Connection))))
#else
	if (NULL == (connection = (struct MHD_Connection *)MHD_calloc_ (1, sizeof (struct MHD_Connection))))
#endif
	{
		eno = errno;
#ifdef HAVE_MESSAGES
...
#ifdef XCONFIG_PATCH
	if (NULL == (connection->addr = (struct sockaddr *)os_malloc (addrlen)))
#else
	if (NULL == (connection->addr = (struct sockaddr *)malloc (addrlen)))
#endif
	{
		eno = errno;
#ifdef HAVE_MESSAGES
                 ...
                 MHD_UNSIGNED_LONG_LONG *timeout)
{
	...
	else
	{
		...
		if (second_left > (time_t)(ULLONG_MAX / 1000)) /* Ignore compiler warning: 'second_left' is always positive. */
			*timeout = ULLONG_MAX;
		else
...
#ifdef XCONFIG_PATCH
		p = (struct pollfd *)os_calloc((2 + num_connections), sizeof (struct pollfd));
#else
		p = (struct pollfd *)MHD_calloc_ ((2 + num_connections), sizeof (struct pollfd));
#endif
		if (NULL == p)
		{
#ifdef HAVE_MESSAGES
...
#ifdef XCONFIG_PATCH
	if (NULL == (daemon = (struct MHD_Daemon *)os_calloc(1, sizeof (struct MHD_Daemon))))
#else
	if (NULL == (daemon = (struct MHD_Daemon *)MHD_calloc_ (1, sizeof (struct MHD_Daemon))))
#endif
		return NULL;
#ifdef EPOLL_SUPPORT
	daemon->epoll_fd = -1;
...
#else
	daemon->options = (enum MHD_FLAG)*pflags;
	pflags = (int *)&daemon->options;
#endif
...
#ifdef XCONFIG_PATCH
		daemon->nnc = (struct MHD_NonceNc *)os_malloc (daemon->nonce_nc_size * sizeof (struct MHD_NonceNc));
#else
...
#if 1
		/* Device binding ~ */
		setsockopt(listen_fd,SOL_SOCKET, SO_BINDTODEVICE, "lo", 2);
#endif
...
#ifdef XCONFIG_PATCH
		daemon->worker_pool = (struct MHD_Daemon *)os_malloc (sizeof (struct MHD_Daemon)
#else
...
{
#ifdef XCONFIG_PATCH
	*ppmtx = (MHD_mutex_ *)os_malloc (sizeof (MHD_mutex_));
#else
	*ppmtx = (MHD_mutex_ *)malloc (sizeof (MHD_mutex_));
#endif
