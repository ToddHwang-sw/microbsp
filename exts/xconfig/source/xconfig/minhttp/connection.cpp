...
try_ready_chunked_body (struct MHD_Connection *connection)
{
	...
	if (0 == connection->write_buffer_size)
	{
		...
		do
		{
			...
			buf = (char *)MHD_pool_allocate (connection->pool,
			                                 size,
			                                 MHD_NO);
		}
		while (NULL == buf);
...
build_header_response (struct MHD_Connection *connection)
{
	...
	{
		data = (char *)MHD_pool_allocate (connection->pool,
		                                  0,
		                                  MHD_YES);
...
parse_cookie_header (struct MHD_Connection *connection)
{
	...
	if (NULL == hdr)
		return MHD_YES;
	cpy = (char *)MHD_pool_allocate (connection->pool,
	                                 strlen (hdr) + 1,
	                                 MHD_YES);
	if (NULL == cpy)
	{
...
parse_connection_headers (struct MHD_Connection *connection)
{
	                                   ...
	                                   MHD_HTTP_HEADER_HOST)) )
	{
		...
		EXTRA_CHECK (NULL == connection->response);
		response =
			(struct MHD_Response *)MHD_create_response_from_buffer (MHD_STATICSTR_LEN_ (REQUEST_LACKS_HOST),
