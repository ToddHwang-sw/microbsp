...
#ifdef __cplusplus
#define NE_none         0
#define NE_content_name 1
#define NE_content_type 2
#define NE_content_filename 4
#define NE_content_transfer_encoding    8
#else
enum NE_State
{
	NE_none = 0,
...
#ifdef XCONFIG_PATCH
	if (NULL == (ret = (struct MHD_PostProcessor *)os_calloc(1, sizeof (struct MHD_PostProcessor) + buffer_size + 1)))
#else
	if (NULL == (ret = (struct MHD_PostProcessor *)MHD_calloc_ (1, sizeof (struct MHD_PostProcessor) + buffer_size + 1)))
#endif
		return NULL;
                  ...
                  char **suffix)
{
	...
	while (0 != *line)
	{
		...
		{
#ifdef XCONFIG_PATCH
			*suffix = os_strdup (&line[strlen (prefix)]);
#else
...
#ifdef XCONFIG_PATCH
		*destination =(char *) os_malloc (vlen);
#else
...
free_unmarked (struct MHD_PostProcessor *pp)
{
	...
	{
#ifdef XCONFIG_PATCH
		os_free (pp->content_transfer_encoding);
#else
	          ...
	          (0 != state_changed) ) )
	{
		...
		case PP_PerformCheckMultipart:
			                                 ...
			                                 MHD_STATICSTR_LEN_ ("multipart/mixed"))))
			{
				...
				pp->nested_boundary =
#ifdef XCONFIG_PATCH
					os_strdup (&pp->nested_boundary[MHD_STATICSTR_LEN_ ("boundary=")]);
#else
