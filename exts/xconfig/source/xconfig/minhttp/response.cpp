                    ...
                    const char *content)
{
	...
	if ( (NULL == response) ||
		...
		return MHD_NO;
#ifdef XCONFIG_PATCH
	if (NULL == (hdr = (struct MHD_HTTP_Header *)os_malloc (sizeof (struct MHD_HTTP_Header))))
#else
	if (NULL == (hdr = (struct MHD_HTTP_Header *)malloc (sizeof (struct MHD_HTTP_Header))))
#endif
		return MHD_NO;
#ifdef XCONFIG_PATCH
	if (NULL == (hdr->header = (char *)os_strdup (header)))
#else
	if (NULL == (hdr->header = (char *)strdup (header)))
#endif
	{
#ifdef XCONFIG_PATCH
		os_free (hdr);
...
#ifdef XCONFIG_PATCH
	if (NULL == (hdr->value = (char *)os_strdup (content)))
#else
	if (NULL == (hdr->value = (char *)strdup (content)))
#endif
	{
#ifdef XCONFIG_PATCH
		os_free (hdr->header);
                                   ...
                                   MHD_ContentReaderFreeCallback crfc)
{
	...
	if ((NULL == crc) || (0 == block_size))
		return NULL;
#ifdef XCONFIG_PATCH
	if (NULL == (response = (struct MHD_Response *)os_calloc(1, sizeof (struct MHD_Response) + block_size)))
#else
	if (NULL == (response = (struct MHD_Response *)MHD_calloc_ (1, sizeof (struct MHD_Response) + block_size)))
#endif
		return NULL;
	response->fd = -1;
#ifdef __cplusplus
                               ...
                               int must_copy)
{
	...
	if ((NULL == data) && (size > 0))
		return NULL;
#ifdef XCONFIG_PATCH
	if (NULL == (response = (struct MHD_Response *)os_calloc(1, sizeof (struct MHD_Response))))
#else
	if (NULL == (response = (struct MHD_Response *)MHD_calloc_ (1, sizeof (struct MHD_Response))))
#endif
		return NULL;
	response->fd = -1;
	if (!MHD_mutex_init_ (&response->mutex))
	...
	{
#ifdef XCONFIG_PATCH
		if (NULL == (tmp = (char *)os_malloc (size)))
#else
		if (NULL == (tmp = (char *)malloc (size)))
#endif
		{
                                 ...
                                 enum MHD_ResponseMemoryMode mode)
{
	...
	ret = MHD_create_response_from_data (size,
	                                      buffer,
                               ...
                               struct MHD_Connection *connection)
{
...
#ifdef XCONFIG_PATCH
	urh = (struct MHD_UpgradeResponseHandle *)os_calloc(1, sizeof (struct MHD_UpgradeResponseHandle));
#else
	urh = (struct MHD_UpgradeResponseHandle *)MHD_calloc_ (1, sizeof (struct MHD_UpgradeResponseHandle));
#endif
	if (NULL == urh)
		if (NULL == urh)
			return MHD_NO;
	...
	printf("%s : read_buffer_size = %d offset=%d \n",__func__,connection->read_buffer_size, connection->read_buffer_offset);
#ifdef HTTPS_SUPPORT
	if (0 != (daemon->options & MHD_USE_TLS) )
	{
		...
		else
		{
			/* Normal case: grab all remaining memory from the
			   ...
			   to another protocol. */
			buf = (char *)MHD_pool_allocate (pool,
			                                 avail,
			                                 MHD_NO);
		}
...
#ifdef XCONFIG_PATCH
	response = (struct MHD_Response *)os_calloc(1, sizeof (struct MHD_Response));
#else
	response = (struct MHD_Response *)MHD_calloc_ (1, sizeof (struct MHD_Response));
#endif
	if (NULL == response)
		return NULL;
	if (!MHD_mutex_init_ (&response->mutex))
