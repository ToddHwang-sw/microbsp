...
/* FIX FIX - Apr. 29. 2009 */
	...
	contents = scew_element_contents (current);
	if (!scew_strcmp(elem, _XT("Path")) && contents) {
		/* Do nothing - why does compare string? */
		//scew_element *e, *root;
		//scew_list *list;
...
#ifdef XCONFIG_PATCH
		new_contents = (XML_Char *)os_calloc (total, 1);
#else
		new_contents = (XML_Char *)calloc (total, 1);
#endif

#if 1   /* FIX FIX - 05.15.2009 */
		if (contents != NULL)
