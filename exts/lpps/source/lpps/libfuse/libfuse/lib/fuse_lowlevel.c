...
static struct fuse_req *fuse_ll_alloc_req(struct fuse_session *se)
{
	...
	req = (struct fuse_req *) os_calloc(1, sizeof(struct fuse_req));
	if (req == NULL) {
...
int fuse_send_reply_iov_noos_free(fuse_req_t req, int error, struct iovec *iov,
			       int count)
{
			  ...
			  int count)
{
	...
	res = fuse_send_reply_iov_noos_free(req, error, iov, count);
	fuse_free_req(req);
	return res;
}
...
int fuse_reply_iov(fuse_req_t req, const struct iovec *iov, int count)
{
	...
	padded_iov = os_malloc((count + 1) * sizeof(struct iovec));
	if (padded_iov == NULL)
...
static void fuse_ll_pipe_os_free(struct fuse_ll_pipe *llp)
{
	close(llp->pipe[0]);
	close(llp->pipe[1]);
	os_free(llp);
...
static struct fuse_ll_pipe *fuse_ll_get_pipe(struct fuse_session *se)
{
	...
	if (llp == NULL) {
		...
		llp = os_malloc(sizeof(struct fuse_ll_pipe));
		if (llp == NULL)
			return NULL;
						      ...
						      size_t count)
{
	...
	fiov = os_malloc(sizeof(fiov[0]) * count);
	if (!fiov)
		return NULL;
			 ...
			 int count)
{
	...
	padded_iov = os_malloc((count + 2) * sizeof(struct iovec));
	if (padded_iov == NULL)
		return fuse_reply_err(req, ENOMEM);
			    ...
			    const void *inarg)
{
	...
	fuse_log(FUSE_LOG_DEBUG,"do_batch_forget!!\n");
...
static void do_read(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	...
	if (req->se->op.read) {
		...
		fi.unique = arg->unique;  /** signalling */
#endif
...
#ifdef EXTRA_WORKAROUND
extern void lpps_ll_interrupt( pthread_t );
#endif
	...
	for (curr = se->list.next; curr != &se->list; curr = curr->next) {
		if (curr->unique == req->u.i.unique) {
			...
			if (func)
				func(curr, data);
#ifdef EXTRA_WORKAROUND
			lpps_ll_interrupt( curr->unique ); /** signalling */
#endif
...
static void do_poll(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	...
	if (req->se->op.poll) {
		...
		if (arg->flags & FUSE_POLL_SCHEDULE_NOTIFY) {
			ph = os_malloc(sizeof(struct fuse_pollhandle));
			if (ph == NULL) {
				  ...
				  const struct fuse_buf *buf, struct fuse_chan *ch)
{
	...
	if (buf->flags & FUSE_BUF_IS_FD) {
		...
		mbuf = os_malloc(tmpbuf.buf[0].size);
		if (mbuf == NULL) {
	...
	if ((buf->flags & FUSE_BUF_IS_FD) && write_header_size < buf->size &&
		...
		err = ENOMEM;
		newmbuf = os_realloc(mbuf, buf->size);
		if (newmbuf == NULL)
			goto reply_err;
		mbuf = newmbuf;
	...
	if (in->opcode == FUSE_READ)
		((struct fuse_read_in *)inarg)->unique = in->unique; /* signalling */
#endif
	...
	if (in->opcode == FUSE_WRITE && se->op.write_buf)
		...
		fuse_ll_ops[in->opcode].func(req, in->nodeid, inarg);  /* FUSE_READ */
				      ...
				      size_t op_size, void *userdata)
{
	...
	se = (struct fuse_session *) os_calloc(1, sizeof(struct fuse_session));
	if (se == NULL) {
...
int fuse_session_mount(struct fuse_session *se, const char *mountpoint)
{
	...
	/* Save mountpoint */
	se->mountpoint = os_strdup(mountpoint);
