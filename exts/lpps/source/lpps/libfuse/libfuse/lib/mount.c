			  ...
			  const char *mnt_opts)
{
	...
	source = os_malloc((mo->fsname ? strlen(mo->fsname) : 0) +
			(mo->subtype ? strlen(mo->subtype) : 0) +
			strlen(devname) + 32);

	type = os_malloc((mo->subtype ? strlen(mo->subtype) : 0) + 32);
	if (!type || !source) {
...
struct mount_opts *parse_mount_opts(struct fuse_args *args)
{
	...
	mo = (struct mount_opts*) os_malloc(sizeof(struct mount_opts));
	if (mo == NULL)
		return NULL;
...
void destroy_mount_opts(struct mount_opts *mo)
{
	os_free(mo->fsname);
	os_free(mo->subtype);
	os_free(mo->fusermount_opts);
	os_free(mo->subtype_opt);
	os_free(mo->kernel_opts);
	os_free(mo->mtab_opts);
	os_free(mo);
}
...
out:
	// EXTRA_WORKAROUND - opts are not in memory space
	//os_free(mnt_opts);
	free(mnt_opts);
	return res;
}
