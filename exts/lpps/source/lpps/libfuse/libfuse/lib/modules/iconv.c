			  ...
			  int fromfs)
{
	...
	newpath = os_malloc(newpathlen + 1);
	if (!newpath)
		return -ENOMEM;
	...
	do {
		...
		if (res == (size_t) -1) {
			...
			inc = (pathlen + 1) * 4;
			newpathlen += inc;
			tmp = os_realloc(newpath, newpathlen + 1);
			err = -ENOMEM;
			if (!tmp)
				goto err;
...
static void iconv_help(void)
{
	...
	charmap = os_strdup(nl_langinfo(CODESET));
	if (old)
				 ...
				 struct fuse_fs *next[])
{
	...
	ic = os_calloc(1, sizeof(struct iconv));
	if (ic == NULL) {
	...
	if (!to[0])
		old = os_strdup(setlocale(LC_CTYPE, ""));
