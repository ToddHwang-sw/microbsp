				...
				struct fusemod_so *so)
{
	...
	mod = os_calloc(1, sizeof(struct fuse_module));
	if (!mod) {
		fuse_log(FUSE_LOG_ERR, "fuse: failed to allocate module\n");
		return -1;
	}
	mod->name = os_strdup(name);
	if (!mod->name) {
...
static int fuse_load_so_module(const char *module)
{
	...
	tmp = os_malloc(strlen(module) + 64);
	if (!tmp) {
		fuse_log(FUSE_LOG_ERR, "fuse: memory allocation failed\n");
		return -1;
	}
	sprintf(tmp, "libfusemod_%s.so", module);
	so = os_calloc(1, sizeof(struct fusemod_so));
	if (!so) {
...
static struct node *alloc_node(struct fuse *f)
{
	return (struct node *) os_calloc(1, get_node_size(f));
}
...
static void node_table_reduce(struct node_table *t)
{
	...
	newarray = os_realloc(t->array, sizeof(struct node *) * newsize);
	if (newarray != NULL)
		t->array = newarray;
...
static char *add_name(char **buf, unsigned *bufsize, char *s, const char *name)
{
	...
	if (s - len <= *buf) {
		...
		newbuf = os_realloc(*buf, newbufsize);
		if (newbuf == NULL)
			return NULL;
		     ...
		     struct fuse_file_info *fi)
{
	...
	if (fs->op.read || fs->op.read_buf) {
		...
		if (fs->op.read_buf) {
			...
			buf = os_malloc(sizeof(struct fuse_bufvec));
			if (buf == NULL)
				return -ENOMEM;
...
int fuse_fs_bmap(struct fuse_fs *fs, const char *path, size_t blocksize,
		 uint64_t *idx)
{
	fuse_log(FUSE_LOG_DEBUG, " %s %ld \n",path, blocksize);

	fuse_get_context()->private_data = fs->user_data;
	if (fs->op.bmap) {
		fuse_log(FUSE_LOG_DEBUG," %s %ld bmap == OK \n", path, blocksize);
		if (fs->debug)
		...
		fuse_log(FUSE_LOG_DEBUG," %s %ld bmap == NULL \n", path, blocksize);
		return -ENOSYS;
	}
}
...
static struct fuse_context_i *fuse_create_context(struct fuse *f)
{
	...
	if (c == NULL) {
		c = (struct fuse_context_i *)
			os_calloc(1, sizeof(struct fuse_context_i));
		if (c == NULL) {
...
static void fuse_delete_context_key(void)
{
	...
	fuse_context_ref--;
	if (!fuse_context_ref) {
		os_free(pthread_getspecific(fuse_context_key));
			     ...
			     struct fuse_file_info *llfi)
{
	...
	dh = (struct fuse_dh *) os_malloc(sizeof(struct fuse_dh));
	if (dh == NULL) {
...
static int extend_contents(struct fuse_dh *dh, unsigned minsize)
{
	if (minsize > dh->size) {
		...
		newptr = (char *) os_realloc(dh->contents, newsize);
		if (!newptr) {
			dh->error = -ENOMEM;
			return -1;
		}
				   ...
				   struct stat *st)
{
	...
	de = os_malloc(sizeof(struct fuse_direntry));
	if (!de) {
		dh->error = -ENOMEM;
		return -1;
	}
	de->name = os_strdup(name);
	if (!de->name) {
		dh->error = -ENOMEM;
		os_free(de);
		return -1;
			      ...
			      size_t size)
{
	...
	if (size) {
		char *value = (char *) os_malloc(size);
		if (value == NULL) {
			reply_err(req, -ENOMEM);
			return;
		}
...
static int locks_insert(struct node *node, struct lock *lock)
{
	...
	if (lock->type != F_UNLCK || lock->start != 0 ||
	    lock->end != OFFSET_MAX) {
		newl1 = os_malloc(sizeof(struct lock));
		newl2 = os_malloc(sizeof(struct lock));
			  ...
			  uint64_t idx)
{
	...
	fuse_log(FUSE_LOG_DEBUG, " %d %s %d %ld \n",ino, path, err, blocksize);
			    ...
			    void *user_data)
{
	...
	fs = (struct fuse_fs *) os_calloc(1, sizeof(struct fuse_fs));
	if (!fs) {
...
static int node_table_init(struct node_table *t)
{
	...
	t->array = (struct node **) os_calloc(1, sizeof(struct node *) * t->size);
	if (t->array == NULL) {
		fuse_log(FUSE_LOG_ERR, "fuse: memory allocation failed\n");
		return -1;
	}
		      ...
		      size_t op_size, void *user_data)
{
	...
	f = (struct fuse *) os_calloc(1, sizeof(struct fuse));
	if (f == NULL) {
...
out_free_root:
	os_free(root);
out_free_id_table:
	os_free(f->id_table.array);
out_free_name_table:
	os_free(f->name_table.array);
out_free_session:
