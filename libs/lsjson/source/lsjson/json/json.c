...
int __sjson_tree_print_internal( sjson_node_t *node , int lvl , int maxlvl , s8 *buff , s32 size )
{
	...
	TDBGP("\tP %-3d   %-3d  ",lvl,node->lvl);
	for (i = 0; i < (node->lvl); i++) 
		TDBGP("  "); 
	TDBGP("<%-5s> %-20s",__node_name(node), symbol_name(node));
	for (i = 0; i < (maxlvl-node->lvl); i++)
		TDBGP("  ");
	TDBGP("%d %c <%-20s> <%-20s> <%-20s>\n",
...
int __sjson_json_print_internal( sjson_node_t *node , s8 *buff, int size, int space )
{
	...
	#define DOSPACE() \
		do{ if (space) { for (i = 0; i < node->lvl; i++) DBGP("  "); } }while(0)
	#define DOSPACE2() \
		do{              for (i = 0; i < node->lvl+1; i++) DBGP("  "); }while(0)
...
int __sjson_xml_print_internal( sjson_node_t *node , s8 *buff, int size, int space )
{
	...
	#define _DOSPACE(space) \
		do{ if (space) { for (i = 0; i < (node->lvl+1+json_xml_setlvl); i++) _DBGP("  "); } }while(0)
	#define _DOSPACE2(space) \
		do{ if (space) { for (i = 0; i < (node->lvl+json_xml_setlvl); i++) _DBGP("  "); } }while(0)
		...
		if (p->type == STMT) {
			...
			if (!path) {
				...
				if (incset) {
					...
					if (!xpath) {
						os_err("FATAL error - path triming error\n");
						os_free(temp);
...
s8 * __sjson_path_name( sjson_node_t * tnode /* should be token node */ )
{
	...
	p = tnode->prev;
	if (!p) {
		os_err("!prev at token\n");
		return symbol_name(tnode);
	}
	...
	if (p->type != STMT) {
		os_err("!stmt/block at token (%s) (%s)\n",__node_name(p), symbol_name(p));
		return symbol_name(tnode);
	}
...
sjson_node_t * __sjson_find_subroot( sjson_node_t * node, s8 *path, int (*compare)(s8 *,s8 *), s8 **subpath, int setindex )
{
	...
	if (setindex != -1) {
		os_dbg("%s: __sjson_get_subtree( %s - SETINDEX_STRCMP ) \n",__func__,spath);
	...
	do {
		*p = 0x0;
		os_dbg("%s: __sjson_get_subtree( %s ) \n",__func__,spath);
...
sjson_node_t * __sjson_get_uptree( sjson_node_t * node ) 
{
	...
	if (p->type != TOKEN)
		os_err("\t%s: weird~ beginning is not token\n",__func__);
	...
	case TOKEN:
		if (r->bros) {
			if (!__sjson_count_visible_nodes(r->bros)) {
				...
				os_dbg("%s: EMPTY BROS (TOKEN/STMT) - %s\n",__func__, symbol_name(r->bros));
				__sjson_free(r->bros);
				r->bros = NULL;
				cnt += 1;
			}
...
int __sjson_delete_json_subtree_internal( sjson_node_t * n )
{
	...
	cst = n->prev;
	if (!cst) {
		os_err("No correspond statement \n");
		return -1;
	}
	...
	if (cst->value != n) {
		...
		if ((cst->type == SET) && 
			...
			os_err("statement error (cst=%s, n=%s)\n", symbol_name(cst), symbol_name(n));
		return 0;
	}
	...
	if (p->type == BLOCK) {
		...
		if (empty_node(p)) {
			temp = p->prev;
			if (!temp) {
				os_dbg("%s: null p->prev(%s) !! strange!! \n",__func__,symbol_name(p));
				return -1;
			}
			if (temp->value == p) {
				p = temp;
				...
				if (!temp) {
					os_dbg("%s: null p->prev(%s) -> prev !! strange!! \n",__func__,symbol_name(p));
					return -1;
				}
				os_dbg("%s: %s CHECK \n",__func__,symbol_name(temp));
				while (temp->bros != p) {
					os_dbg("%s: %s->bros is not %s \n",__func__,symbol_name(temp),symbol_name(p));
					temp = temp->bros;
					if (!temp)
						break;
				}
				if (!temp) {
					os_dbg("%s: failed in searching temp !! strange!! \n",__func__);
					return -1;
				}
				/* 
				 *
...
int sjson_parse( sjson_table_t *tbl, u8 *mem )
{
	...
	while (*sp) {
		...
		case 'A'...'Z':  /* --+        */
			...
			while (DIGIT(*sp) || ALPHA(*sp)) {
				if (tokenp >= TOKENSZ) {
					os_err("Too long string\n");
					goto parse_error;
				}
				token[ tokenp++ ] = *sp++;
			}	
...
sjson_node_t * sjson_find_json( sjson_node_t *r , s8 *path , int mode )
{
	...
	if (pin_sn) { /* exceptional case */
		...
		if (!nt) {
			os_err("%s: sjson_new_node( TOKEN %s ) failed \n",__func__, pin_sn);
			os_free(pin_sn);
			__sjson_free( n ); 
			return NULL;
		}
...
sjson_node_t * sjson_add_json( sjson_node_t * n, sjson_node_t * a, s8 *spath )
{
	...
	if (parent) {
		...
		os_dbg("%s: [%s] + [%s] : \n", __func__, path , nsubpath); /* parent is SET */
		{
			...
			if (xindex != -1) 
				os_dbg("%s: SET-PARENT PATH=<%s> INDEX=<%d>\n",__func__,path,xindex);
		}
		...
		if (!child) {
			...
			if (!v) {
				os_dbg("%s: PARENT->VALUE->VALUE = NULL\n",__func__);
				__sjson_free(pchild);
				goto __error_set_elem_add;
			}
			v = v->bros;
			...
			if (!v) {
				os_dbg("%s: PARENT->VALUE->VALUE->BROS = NULL\n",__func__);
				__sjson_free(pchild);
				goto __error_set_elem_add;
			}
		...
		if ((multiple_node(trace) ||
			...
			if ((xindex != -1) && 
				...
				if (v) {
					...
					if (v && (v->type == STMT)) {
						trace = v; /* move trace */
						os_dbg("%s: TRACE MOVE TO %s \n",__func__,symbol_name(trace));
					}
				}
			} else {
				os_dbg("%s: BLOCK ATTACH %d/(%d+%d) \n",__func__,cnt,xindex,premember_count);
